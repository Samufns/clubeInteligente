...
.enddata
.head 3 +  Functional Class: cDBVCLUBE
.head 4 -  Description:
.head 4 +  Derived From
...
.head 9 -  Return FALSE
.head 8 -  Call SqlPrepareAndExecute( hSqlParam, 'Insert into DBVCLUBE 
( ID,  NOME,  ATIVO )
 Values 
( :nID,  :sNOME,  :nATIVO ) ' )
.head 8 +  If bUpdate
...
.head 8 -  Call SqlSetIsolationLevel( hSqlParam, 'RL' )
.head 8 -  Call SqlPrepareAndExecute( hSqlParam, 'Delete From DBVCLUBE
Where ID = :nID' )
...
.head 7 +  If VerificaCampos() And VerificaCamposCustom( )
.head 8 -  Call SqlSetIsolationLevel( hSqlParam, 'RL' )
.head 8 -  Call SqlPrepareAndExecute( hSqlParam, 'Update DBVCLUBE
Set NOME = :sNOME,
ATIVO = :nATIVO 
Where ID = :nID ' )
.head 8 -  Call SqlSetIsolationLevel( hSqlParam, 'RL' )
...
.head 7 +  If SalStrLength( sNOME  ) > 150
...
.head 7 -  Call SqlPrepareAndExecute( hSqlParam, 'Insert into SSTLogDetalhe ( SISTEMA, SEQNO, OBJETO, INDICE, VALOR )
 values (:sSistemaGlobal, :nSeqNo, \'ATIVO\', 2, ' || SalNumberToStrX( nATIVO, 7 )  || '  )'  )
.head 7 -  Return TRUE
...
.head 6 +  Actions
.head 7 -  Call SqlPrepareAndExecute( hSqlParam, 'Select MAX(ID) Into :nMAXID From DBVCLUBE' )
...
.head 3 +  Functional Class: cDBVCLUBE_Vetor
.head 4 -  Description:
.head 4 +  Derived From
...
.head 5 -  FunctionalVar: oDBVCLUBE[*]
.head 6 -  Class: cDBVCLUBE
.head 4 +  Functions
.head 5 +  Function: Reset
.head 6 -  Description:
.head 6 -  Returns
...
.head 6 -  Local variables
.head 6 +  Actions
.head 7 -  Call SalArraySetUpperBound( oDBVCLUBE , 1, -1 )
...
Select ID,  NOME,  ATIVO
Into  :oDBVCLUBE[nInd].nID,  :oDBVCLUBE[nInd].sNOME,  :oDBVCLUBE[nInd].nATIVO 
From DBVCLUBE ' || sWhere || ' 
...
.head 8 -  Set oDBVCLUBE[nVetorPos].nID = SalStrToNumber( sValor )
.head 7 +  Else If sNomeVar = 'sNOME'
.head 8 -  Set oDBVCLUBE[nVetorPos].sNOME = sValor
.head 7 +  Else If sNomeVar = 'nATIVO'
.head 8 -  Set oDBVCLUBE[nVetorPos].nATIVO = SalStrToNumber( sValor )
.head 7 -  Return TRUE
...
.head 6 +  Actions
.head 7 +  If sNomeVar = 'nID'
.head 8 -  Call vParamExport.SetNumber( oDBVCLUBE[nVetorPos].nID , VT_R8 )
.head 7 +  Else If sNomeVar = 'sNOME'
.head 8 -  Call vParamExport.SetString( oDBVCLUBE[nVetorPos].sNOME)
.head 7 +  Else If sNomeVar = 'nATIVO'
.head 8 -  Call vParamExport.SetNumber( oDBVCLUBE[nVetorPos].nATIVO , VT_R8 )
...
.head 7 +  If nIndice >= 0
.head 8 -  Set sCondicao = VisStrSubstitute( sConditionsLog[nIndice], '%COLUNA', 'oDBVCLUBE[ nVetorPos ].' || sColuna )
...
.head 8 -  Return oDBVCLUBE[nLinha].Insert( hSqlParam )
.head 7 +  Else If sOperacao = '22'
.head 8 -  Return oDBVCLUBE[nLinha].Delete( hSqlParam )
.head 7 +  Else If sOperacao = '33'
.head 8 -  Return oDBVCLUBE[nLinha].Update( hSqlParam )
