...
.enddata
.head 3 +  Functional Class: cDBVCLASSE
.head 4 -  Description:
.head 4 +  Derived From
...
'Select ID,  NOME,  IDADE,  TIPO
Into  :nID,  :sNOME,  :nIDADE,  :nTIPO 
From DBVCLASSE 
Where ID = :nID_p' )
...
.head 7 -  Set nTIPO = oOrigem_p.nTIPO
.head 7 -  Set bInitialized = FALSE
...
.head 9 -  Return FALSE
.head 8 -  Call SqlPrepareAndExecute( hSqlParam, 'Insert into DBVCLASSE 
( ID,  NOME,  IDADE,  TIPO )
 Values 
( :nID,  :sNOME,  :nIDADE,  :nTIPO ) ' )
.head 8 +  If bUpdate
...
.head 8 -  Call SqlSetIsolationLevel( hSqlParam, 'RL' )
.head 8 -  Call SqlPrepareAndExecute( hSqlParam, 'Delete From DBVCLASSE
Where ID = :nID' )
...
.head 7 +  If VerificaCampos() And VerificaCamposCustom( )
.head 8 -  Call SqlSetIsolationLevel( hSqlParam, 'RL' )
.head 8 -  Call SqlPrepareAndExecute( hSqlParam, 'Update DBVCLASSE
Set NOME = :sNOME,
IDADE = :nIDADE,
TIPO = :nTIPO 
Where ID = :nID ' )
...
.head 7 -  Call SqlPrepareAndExecute( hSqlParam, 'Insert into SSTLogDetalhe ( SISTEMA, SEQNO, OBJETO, INDICE, VALOR )
 values (:sSistemaGlobal, :nSeqNo, \'IDADE\', 2, ' || SalNumberToStrX( nIDADE, 7 )  || '  )'  )
.head 7 -  Call SqlPrepareAndExecute( hSqlParam, 'Insert into SSTLogDetalhe ( SISTEMA, SEQNO, OBJETO, INDICE, VALOR )
 values (:sSistemaGlobal, :nSeqNo, \'TIPO\', 3, ' || SalNumberToStrX( nTIPO, 7 )  || '  )'  )
.head 7 -  Return TRUE
...
.head 6 +  Actions
.head 7 -  Call SqlPrepareAndExecute( hSqlParam, 'Select MAX(ID) Into :nMAXID From DBVCLASSE' )
...
.head 3 +  Functional Class: cDBVCLASSE_Vetor
.head 4 -  Description:
.head 4 +  Derived From
...
.head 5 -  FunctionalVar: oDBVCLASSE[*]
.head 6 -  Class: cDBVCLASSE
.head 4 +  Functions
.head 5 +  Function: Reset
.head 6 -  Description:
.head 6 -  Returns
...
.head 6 -  Local variables
.head 6 +  Actions
.head 7 -  Call SalArraySetUpperBound( oDBVCLASSE , 1, -1 )
...
Select ID,  NOME,  IDADE,  TIPO
Into  :oDBVCLASSE[nInd].nID,  :oDBVCLASSE[nInd].sNOME,  :oDBVCLASSE[nInd].nIDADE,
:oDBVCLASSE[nInd].nTIPO 
From DBVCLASSE ' || sWhere || ' 
...
.head 8 -  Set oDBVCLASSE[nVetorPos].nID = SalStrToNumber( sValor )
.head 7 +  Else If sNomeVar = 'sNOME'
.head 8 -  Set oDBVCLASSE[nVetorPos].sNOME = sValor
.head 7 +  Else If sNomeVar = 'nIDADE'
.head 8 -  Set oDBVCLASSE[nVetorPos].nIDADE = SalStrToNumber( sValor )
.head 7 +  Else If sNomeVar = 'nTIPO'
.head 8 -  Set oDBVCLASSE[nVetorPos].nTIPO = SalStrToNumber( sValor )
.head 7 -  Return TRUE
...
.head 6 +  Actions
.head 7 +  If sNomeVar = 'nID'
.head 8 -  Call vParamExport.SetNumber( oDBVCLASSE[nVetorPos].nID , VT_R8 )
.head 7 +  Else If sNomeVar = 'sNOME'
.head 8 -  Call vParamExport.SetString( oDBVCLASSE[nVetorPos].sNOME)
.head 7 +  Else If sNomeVar = 'nIDADE'
.head 8 -  Call vParamExport.SetNumber( oDBVCLASSE[nVetorPos].nIDADE , VT_R8 )
.head 7 +  Else If sNomeVar = 'nTIPO'
.head 8 -  Call vParamExport.SetNumber( oDBVCLASSE[nVetorPos].nTIPO , VT_R8 )
...
.head 7 +  If nIndice >= 0
.head 8 -  Set sCondicao = VisStrSubstitute( sConditionsLog[nIndice], '%COLUNA', 'oDBVCLASSE[ nVetorPos ].' || sColuna )
...
.head 8 -  Return oDBVCLASSE[nLinha].Insert( hSqlParam )
.head 7 +  Else If sOperacao = '22'
.head 8 -  Return oDBVCLASSE[nLinha].Delete( hSqlParam )
.head 7 +  Else If sOperacao = '33'
.head 8 -  Return oDBVCLASSE[nLinha].Update( hSqlParam )
